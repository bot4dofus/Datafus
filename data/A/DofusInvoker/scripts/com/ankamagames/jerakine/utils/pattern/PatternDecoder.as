package com.ankamagames.jerakine.utils.pattern
{
   ...
   public class PatternDecoder
   {
      ...
      private static const ITERATION_LIMIT:uint = 1000;
      
      protected static const _log:Logger = Log.getLogger(getQualifiedClassName(PatternDecoder));
      ...
      private static function decodeDescription(baseStr:String, aParams:Array) : String
      {
         var nextSharp:int = 0;
         var nextTilde:int = 0;
         var nextBrace:int = 0;
         ...
         var pos2:int = 0;
         var n2:Number = NaN;
         var aStr:String = findOptionnalDices(baseStr,aParams);
         ...
         while(true)
         {
            ...
            counter++;
            if(!(counter < ITERATION_LIMIT && (nextSharp != -1 || nextTilde != -1 || nextBrace != -1 || nextBracket != -1)))
            {
               if(counter >= ITERATION_LIMIT)
               {
                  _log.error("Max iteration limit reached (" + ITERATION_LIMIT + ")! Current string is \"" + aStr + "\", initial string is \"" + baseStr + "\".");
                  return "";
               }
               return aStr;
            }
